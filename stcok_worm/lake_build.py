...
产物（默认根 D:/work Buddy GZ/Claw/stockworm，可用 --lake 或 STOCKWORM_LAKE 覆盖）:
  ...
  metadata/industry_map.csv                行业映射（多源：东财板块优先 + 巨潮兜底，列 eastmoney_industry/cninfo_industry）
...
依赖: akshare, stcok_worm.fundamentals_ext, stcok_worm.regulatory, stcok_worm.industry_map, stcok_worm.datalake
"""
...
# ---------- 5. 行业映射（委托 industry_map 多源模块） ----------
def _migrate_old_industry_progress(meta, prog_path):
    """首次运行：把旧 ind_map_progress.json 的 cninfo_done/done 迁到新 ind_done，复用已有抓取。"""
    if prog_path.exists():
        return
    old = meta / "ind_map_progress.json"
    if not old.exists():
        return
    try:
        data = json.loads(old.read_text(encoding="utf-8"))
        done = set(data.get("cninfo_done", [])) | set(data.get("done", []))
        if done:
            prog_path.write_text(json.dumps({"ind_done": sorted(done)}, ensure_ascii=False), encoding="utf-8")
            logger.info("迁移旧行业映射进度: %d 只 -> %s", len(done), prog_path)
    except Exception as exc:
        logger.warning("迁移旧行业映射进度失败(将重新抓取): %s", exc)


def build_industry_map(source: str = "auto", min_bars: int = 250):
    """行业映射 → metadata/industry_map.csv（多源，默认不依赖单一东财）。

    委托 stcok_worm.industry_map 多源模块：
      source='auto'（默认） 东财行业板块优先，未覆盖/东财端点失效的标的用巨潮兜底；
      source='eastmoney'    仅东财板块；
      source='cninfo'       仅巨潮逐只（独立 host，最稳）。
    落盘列：code, eastmoney_industry（始终填充，缺口用巨潮补）, cninfo_industry（巨潮真值）。
    带断点续传（industry_progress.json 键 ind_done）+ 增量落盘；首次自动迁移旧进度。
    """
    from stcok_worm import industry_map as imod
    import pandas as pd
    _, meta, _ = _paths()
    out = meta / "industry_map.csv"
    prog_path = meta / "industry_progress.json"

    _migrate_old_industry_progress(meta, prog_path)

    codes = list_codes_from_daily(min_bars)
    logger.info("行业映射: 待评估 %d 只 (source=%s)", len(codes), source)
    n = 0
    for chunk in imod.industry_map_all(codes, source=source, progress_path=str(prog_path)):
        if chunk is not None and not chunk.empty:
            chunk.to_csv(out, mode="a", index=False,
    ...
    if out.exists():
        ...
        logger.info("行业映射数据湖完成: %d 只 -> %s", len(df), out)
        print(df.head(10).to_string())
    else:
        logger.warning("未生成任何行业映射")
...
def main():
    ...
    ap.add_argument("--source", default="eastmoney", choices=["eastmoney", "cninfo", "auto"],
                    help="源选择：--only industry 默认 auto(东财+巨潮兜底)；--only regulatory 默认 eastmoney")
